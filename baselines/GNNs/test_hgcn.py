"""
test_hgcn.py — FISH HypergraphConv benchmark: 12 conditions x N seeds.

Conditions
----------
  {HGCN, HGCN + compartment} x {+type-he, no-type-he} x {regression, CORN, CORAL}
...
Outputs
-------
    results_hgcn.json / results_hgcn.csv  (per-run + summary table to stdout).
"""
    ...
    for type_he in (False, True):
        for comp in (False, True):
            for decoder in ("regression", "corn", "coral"):
                ...
                if comp:
                    parts.append("+c")
                parts.append(f"/{decoder}")
                conditions.append(Condition(
                    name=" ".join(parts).replace(" /", "/"),
                    use_compartment=comp,
                    use_type_hyperedges=type_he,
                    order_mode=decoder,
                ))
    return conditions
...
def load_compartment_embeddings(path: str) -> dict:
    ...
    if suffix == ".npz":
        ...
        return {k: npz["vectors"].astype(np.float32)[i] for i, k in enumerate(keys)}
    ...
    if suffix in (".tsv", ".txt"):
        ...
        return out
    raise ValueError(f"Unrecognised compartment-embeddings format: {suffix}")
...
def run_one(
    ...
    model = FISHGCN(
        ...
        n_heads=n_heads,
        use_type_hyperedges=condition.use_type_hyperedges,
        order_mode=condition.order_mode,
        n_order_bins=n_order_bins,
    ...
    metrics["use_type_hyperedges"] = condition.use_type_hyperedges
...
def main():
    ...
    parser.add_argument("--time-target", default="min_max",
    ...
    parser.add_argument("--use-attention", action="store_true",
                        help="Use the attention variant of HypergraphConv.")
    parser.add_argument("--n-heads", type=int, default=1,
                        help="Attention heads (only used with --use-attention).")
    parser.add_argument("--out-json", default="results_hgcn.json")
    parser.add_argument("--out-csv", default="results_hgcn.csv")
    ...
    if args.device is not None:
        ...
        if not torch.cuda.is_available():
            parser.error("--gpu requested but CUDA unavailable.")
        device = "cuda"
    elif args.cpu:
        device = "cpu"
    else:
    ...
    ) if args.biopax else load_pickled_graph(args.pickle)
    ...
    if args.compartment_embeddings:
        ...
        print(f"[main] Loaded compartment embeddings: "
              f"{len(compartment_embeddings)} terms, "
              f"dim={len(next(iter(compartment_embeddings.values())))}")
    else:
        print("[main] No compartment-embeddings; +c conditions use one-hot vocab.")
    ...
    print("\n" + "=" * 72)
    print(f"SUMMARY: {args.pathway_name}, semi-inductive, {args.seeds} seeds")
    print("=" * 72)
