"""
tune_tgat.py — Optuna hyperparameter sweep for the FISH TGAT baseline.

Tunes the headline condition (TGAT + compartment / coral) on the Immune
semi-inductive split. Same multi-task score objective as tune.py so results
are comparable across RGCN and TGAT sweeps.

Outputs
-------
  best_params_tgat.json   — best hyperparameters found.
  optuna_history_tgat.csv — per-trial metrics.
...
def multi_task_score(metrics: dict) -> float:
    """
    Multi-task score matching tune.py so sweeps are directly comparable.
...
def make_objective(G, args, device):
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        max_neighbors = trial.suggest_categorical("max_neighbors", [10, 20, 50])
        ...
        # CORAL loss scale fix (same as RGCN tuner).
...
def main():
    ...
    parser.add_argument("--out-json", default="best_params_tgat.json")
    parser.add_argument("--out-csv", default="optuna_history_tgat.csv")
    ...
    if args.study_name is None:
        args.study_name = (
            f"fish_{args.pathway_name}_tgat+c_coral"
            .replace(" ", "_").replace("/", "_")
        )
