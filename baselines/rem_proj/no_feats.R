...
events_raw <- read_csv("../.cache/edges.csv", show_col_types = FALSE)
...
## Endogenous-only specs (no features)
fit_choice <- function(etype) {
  nn <- paste0("net_", etype); dn <- paste0("dep_", etype)
  specs <- list(
    sprintf("%s ~ inertia(%s) + recip(%s) + trans(%s)", dn, nn, nn, nn),
    sprintf("%s ~ inertia(%s) + recip(%s)",             dn, nn, nn),
    sprintf("%s ~ inertia(%s) + trans(%s)",             dn, nn, nn),
...
fit_rate <- function(etype) {
  nn <- paste0("net_", etype); dn <- paste0("dep_", etype)
  specs <- list(
    sprintf("%s ~ indeg(%s) + outdeg(%s)", dn, nn, nn),
...
## Endogenous-only scoring with running adjacency
...
raw_path <- "../.cache/rem_results_raw_endogenous.csv"
...
summary_path <- "../.cache/rem_results_summary_endogenous.csv"
